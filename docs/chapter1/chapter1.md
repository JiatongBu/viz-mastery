...
在谈论所有的可视化美学之前，必须要记住，可视化的最终目的是为了向读者**传递信息**。而不同类型的美学属性适用于不同类型的变量。例如，颜色和形状适合分类变量，而大小更适合连续变量。数据量的多少也会产生影响：如果数据量很大，可能难以区分不同的组，我们可以采用ggplot2中的faceting，或者seaborn的FacetGrid。
...
**合适的字体**  
配图的字体在所投期刊中可能会有规定，若是没有规定，可以使用Arial或Helvetica。


**合适的颜色搭配**
No.1 色彩的数量  
科研论文插图配色时应尽量精简。主色调建议选择1-2个色系，内容较多时也可以选择个色系。其他内容可以使用同一色系的不同饱和度、明度来进行区分。
No.2 色彩的选择  
...
例如，我们可以看到，在[这篇论文](https://www.nature.com/articles/s41558-024-02084-1)当中，使用明显有对比度的三个色系来区分不同的参数，可以有效增加图片的可读性。
...
1. 19世纪早期到中期：普莱费尔、南丁格尔、斯诺、米纳尔

最早用于表示数据的图形既简洁又抽象，同时主要是作为交流的工具。考虑到19世纪早期到中期，即使是简单的家居用品也常带有装饰性，这些早期图表的抽象性显得尤为突出。与当时的许多地图相比，约翰·斯诺和查尔斯·米纳尔的地图简洁到了极点。



这一时期还培养了威廉·普莱费尔 （William Playfair），他被广泛认为是统计图形之父，包括我们今天使用的最流行的图形（折线图、条形图、圆图和饼图）。普莱费尔是一位苏格兰工程师和政治经济学家，他于 1786 年出版了《商业和政治地图集》。下图所示的 Playfair （1821） 的图表显示了 1565 年至 1820 年 250 年间小麦的价格、每周工资和在位君主。



这些图表的目标是传达信息，而非进行分析。斯诺的霍乱地图经常被错误地用作视觉分析的例子，实际上它是为了说服人们而绘制的。同样，弗洛伦斯·南丁格尔关于克里米亚战争死亡人数的图表用于说明她的观点，即改善卫生条件可以挽救许多生命。威廉·普莱费尔则通过图表展示了英国与其贸易伙伴的贸易平衡。
2. 1920-30年代：诺伊拉特

20世纪最初的30年间，奥托·诺伊拉特设计了一种视觉语言——ISOTYPE，不仅使数字更易于阅读，还传递了其含义。例如，想要展示工人和工厂的统计数据？就用小工人和小工厂的图标，每个图标代表一定数量的对应对象，从而便于比较和读取数据。

诺伊拉特的目标是向人们普及世界知识。ISOTYPE是“国际图形化教育系统”（International System of Typographic Picture Education）的缩写。他的图表旨在无需他人解释即可自我呈现。而南丁格尔和普莱费尔假设会有解释性文字或创作者本人来支持图表的观点。诺伊拉特则试图让他的图表成为自给自足、无需说明的作品。

3. 1960-70年代：贝尔坦与图基

三十年后，重心从展示转向分析，图表中的解释性部分再次消失。约翰·图基特别关注通过数据的图形化表现可以学到什么，而不是它是否适合作为演示工具。贝尔坦则同时使用图形进行分析和交流，尽管他更多用于展示的图表主要是地图。

图基发明了许多不同类型的图表，包括箱线图、气泡图、雷达图等。贝尔坦除了其具有开创性的理论工作，还开发了可重新排序的矩阵，这是一种简单而强大的工具，用于数据聚类分析。这也是可视化中交互性使用的早期实例之一。

4. 1970-80年代：霍尔姆斯

20世纪70年代末到80年代初出现了一种新趋势：复杂的信息图开始用于传递数据。奈杰尔·霍尔姆斯是这一类可视化最著名的设计师之一。

霍尔姆斯使用“解释性图形”这一术语，不仅更少被误用，也更清楚地描述了其目标：解释数据及其背景。此外，霍尔姆斯还希望吸引读者的注意力并提供娱乐性。其结果是非常复杂且独特的信息图，但始终基于真实数据。
1980年代：塔夫特

与霍尔姆斯形成鲜明对比的是，爱德华·塔夫特提倡极简和未加修饰的风格，强调展示数据本身。尽管塔夫特经常谈论展示信息，他的关注点显然是用于分析的数据呈现。与当代信息可视化研究的不同之处在于，塔夫特几乎完全讨论静态表现形式（通常在纸张上，因其信息密度高的潜力），以供用户探索数据并解答问题。

塔夫特的影响在当今可视化领域显而易见，他的名字常被引用来批评复杂的信息图。塔夫特和霍尔姆斯代表了装饰性光谱的两端，而科学界已较为充分地探索了塔夫特的一端，但在霍尔姆斯的方向上仍有许多工作要做。
...
与此同时，学术界的可视化研究专注于数据分析和探索，却几乎完全忽视了信息图和解释性图表。虽然当前研究工作有其价值，但也错失了重要的机会。
