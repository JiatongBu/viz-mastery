...
我们要做一张良好的可视化的图片，往往需要具备良好的读者思维。比如说，我们需要明确可视化的目标受众的背景，他的专业知识水平，是否熟悉数据分析或统计学等等。对于非专业受众，我们还需要避免使用过多术语或复杂图形。
...
科研绘图时需特别注意专业规范的问题，有几类大家容易犯的问题：
- 可复现与完整性：在论文中，科研图表不仅需要用于结果展示，还可能被引用或复用在后续研究中。同时，我们也要提供必要的图表说明,包括数据来源、处理方法等关键信息。
- 版权合规：在使用他人图表或素材时,应取得合法授权或使用许可,并在图注中清晰标明出处。  
- 期刊规范：图表的设计和内容需符合目标期刊或会议的投稿规范,例如版面尺寸、颜色模式和字体选择等等。  
...
在AI时代，已经有很多工具可以辅助我们进行配色了，例如[radix](https://www.radix-ui.com/colors/custom)和[fffuel](https://www.fffuel.co/pppalette/)这两个网站。
...
如今是信息爆炸的年代，信息图大量涌现，也混合了许多不同的风格。其中，有很多风格夸张的图表，这些图表往往更突出。这些信息图常被用来吸引注意力并为普通文章带来流量。然而，不少的现代信息图缺乏准确性和深度。与我们刚刚提到的1980年代的信息图相比，虽然复杂，但只是增加了视觉效果，却几乎无实际用途，同时在数据使用上相当随意。这是相当普遍的问题。
